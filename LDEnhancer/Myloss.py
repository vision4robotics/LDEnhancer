...
class tracker_perception_loss(nn.Module):
    def __init__(self):
        super(tracker_perception_loss, self).__init__()
...
class Gray_loss(nn.Module):
    ...
    def forward(self, x, y=0.2):
        r, g, b = torch.split(x, 1, dim=1)
        w_r =torch.exp(-torch.pow((r-0.5),2)/(2*y**2))
        w_g =torch.exp(-torch.pow((g-0.5),2)/(2*y**2))
        w_b =torch.exp(-torch.pow((b-0.5),2)/(2*y**2))
        Igray = (w_r*r+w_g*g+w_b*b)/3
        Igray = torch.cat([Igray,Igray,Igray],1)
